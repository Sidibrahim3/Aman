...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query("SELECT t FROM Transaction t WHERE t.createDate >= :startOfDay AND t.createDate <= CURRENT_TIMESTAMP AND (t.isDeleted = false OR t.isDeleted IS NULL) AND t.agency.id = :agencyId")
    List<Transaction> findTransactionsForTodayByAgencyId(@Param("startOfDay") LocalDateTime startOfDay, @Param("agencyId") Long agencyId);
